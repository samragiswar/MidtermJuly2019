...
public class CnnAPI {
    ...
    public static void main(String[] args) throws MalformedURLException, IOException {
        String sURL = "https://newsapi.org/v2/top-headlines?sources=cnn&apiKey=8c3dfbaa002c483ea9d3cdde08634dd3";
        CccApiMainClass methods = null;
        List<CccApiMainClass> newsList = new ArrayList<>();
        ...
        for (int i = 0; i < jsonArray.get(0).getAsJsonArray().size(); i++) {
            try {
                ...
                methods = new CccApiMainClass(source, author, title, description, u, urlToImage, publisherAt, content);
        ...
        for (CccApiMainClass entry : newsList) {
        ...
        try {List<CccApiMainClass> st1List = connectToSqlDB.newReadListFromSqlDB(newsList, "CNNApi");

            for (CccApiMainClass entry : st1List) {
                System.out.println(entry.getSource() + " " + entry.getAuthor() + " " + entry.getTitle() + " " + entry.getDescription() + " " + entry.getUrl() + " " + entry.getUrlToImage() + " " + entry.getPublisherAt() + " " + entry.getContent());
            }
        } catch (Exception ex) {}
